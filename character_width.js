...
function process_bitmap(toggle_event, bitmap) {
    ...
    for(let chars_line of bitmap.chars) {
        if (chars_line.length > atlas_width) atlas_width = chars_line.length;
    }
    ...
    for (let cy = 0; cy < atlas_height; cy++) {
        for (let cx = 0; cx < atlas_width && cx < bitmap.chars[cy].length; cx++) {
            ...
            console.info(`U${char.codePointAt(0).toString(16).padStart(4, '0')}</td><td>${char}</td><td><img alt="Broken ${char} glyph" src="${glyph.toDataURL("image/png")}"></td><td>${real_char_width}</td>`)
        }
    }
